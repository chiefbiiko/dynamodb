...
export function deriveConfig(conf: ClientConfig = {}): Doc {
  ...
  if (_conf.profile || !_conf.region || (typeof _conf.credentials !== "function" && (!_conf.credentials.accessKeyId || !_conf.credentials.secretAccessKey))) {
    const got: Doc = get({ profile: _conf.profile || "default" });
    
    _conf.credentials = { ..._conf.credentials, ...got.credentials }
